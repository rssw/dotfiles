...
PKG_OPTIONAL_LSP_DEFAULT="nodejs npm python3-pip python3-venv pipx golang-go"
...
install_template_if_missing() {
  ...
  if [ -e "$template_target_path" ] || [ -L "$template_target_path" ]; then
    if [ -f "$template_source_path" ] && cmp -s "$template_source_path" "$template_target_path" 2>/dev/null; then
      log "Already matches template: $template_target_path"
      return 0
    fi
...
install_lsp_servers() {
  ...
  if command -v pipx >/dev/null 2>&1; then
    run_in_home pipx install --force jedi-language-server
  elif command -v python3 >/dev/null 2>&1; then
    venv_dir=$HOME_DIR/.local/share/nvim/lsp/jedi-language-server
    run_in_home python3 -m venv "$venv_dir"
    run_in_home "$venv_dir/bin/pip" install --upgrade pip
    run_in_home "$venv_dir/bin/pip" install jedi-language-server
  fi
}
